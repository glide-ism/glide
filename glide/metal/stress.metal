// Auto-translated from cuda/stress.cu by the metal port (mechanical: pure helpers).
...
struct SigmaNormalJacobian {
    ...
    inline
    float apply_jvp(const thread SigmaNormalStencil& dot) const {
...
struct SigmaShearJacobian {
    ...
    inline
    float apply_jvp(const thread SigmaShearStencil& dot) const {
...
struct TauBxJacobian {
    ...
    inline
    float apply_jvp(const thread TauBxStencil& dot) const {
        return d_u * dot.u +
	       d_v_tl * dot.v_tl +
	       d_v_tr * dot.v_tr +
	       d_v_bl * dot.v_bl +
   ...
   TauBxStencil s )
{
    ...
    float unorm_sq_pow = pow(unorm_sq + s.u_reg,(s.m - 1.0f)/2.0f);
    float unorm_sq_deriv = (s.m - 1.0f)/2.0f * pow(unorm_sq + s.u_reg,(s.m - 1.0f)/2.0f - 1.0f);
...
struct TauByJacobian {
    ...
    inline
    float apply_jvp(const thread TauByStencil& dot) const {
        return d_v * dot.v +
	       d_u_tl * dot.u_tl +
	       d_u_tr * dot.u_tr +
	       d_u_bl * dot.u_bl +
...
struct TauDxJacobian {
    ...
    inline
    float apply_jvp(const thread TauDxStencil& dot) const {
...
struct TauDyJacobian {
    ...
    inline
    float apply_jvp(const thread TauDyStencil& dot) const {
