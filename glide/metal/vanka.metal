    ...
    float dx, float dt, int ny, int nx, int i, int j)
{
    ...
    float eta_r  = eta_at(i, j + 1, u, v, B, n, eps_reg, dx, ny, nx);
    float eta_b  = eta_at(i + 1, j, u, v, B, n, eps_reg, dx, ny, nx);
    float eta_br = eta_at(i + 1, j + 1, u, v, B, n, eps_reg, dx, ny, nx);
...
kernel void vanka_smooth(
    device atomic_uint* delta_u [[buffer(0)]],
    device atomic_uint* delta_v [[buffer(1)]],
    device float* delta_H [[buffer(2)]],
    device float* mask [[buffer(3)]],
    device const float* u [[buffer(4)]],
    device const float* v [[buffer(5)]],
    device const float* H [[buffer(6)]],
...
)
{
    ...
    int newton_steps = (int)params[15];
    float relaxation = params[16];
    float ssa_damping = params[17];
    float mc_damping = params[18];
	...
	atomic_add_float(&delta_u[i * (nx + 1) + j],       0.5f*(u_l - u_l_prev));
	atomic_add_float(&delta_u[i * (nx + 1) + j + 1],   0.5f*(u_r - u_r_prev));
	atomic_add_float(&delta_v[i * nx + j],             0.5f*(v_t - v_t_prev));
	atomic_add_float(&delta_v[(i + 1) * nx + j ],      0.5f*(v_b - v_b_prev));
...
kernel void vanka_smooth_adjoint(
    device atomic_uint* lambda_u_out [[buffer(0)]],
    device atomic_uint* lambda_v_out [[buffer(1)]],
    device atomic_uint* lambda_H_out [[buffer(2)]],
    ...
    device const float* r_adj_u [[buffer(8)]],
    device const float* r_adj_v [[buffer(9)]],
    device const float* r_adj_H [[buffer(10)]],
    device const float* bed [[buffer(11)]],
    device const float* B [[buffer(12)]],
    device const float* beta [[buffer(13)]],
    device const float* gamma [[buffer(14)]],
...
)
{
    ...
    int stride = (int)params[13];
    int halo = (int)params[14];
    float ssa_damping = params[15];
    float mc_damping = params[16];
	...
	atomic_add_float(&lambda_u_out[i * (nx + 1) + j],      0.5f*delta_lambda[0]);
	atomic_add_float(&lambda_u_out[i * (nx + 1) + j + 1],  0.5f*delta_lambda[1]);
	atomic_add_float(&lambda_v_out[i * nx + j],            0.5f*delta_lambda[2]);
	atomic_add_float(&lambda_v_out[(i + 1) * nx + j ],     0.5f*delta_lambda[3]);
	atomic_add_float(&lambda_H_out[i * nx + j],                 delta_lambda[4]);
    
    }
}

kernel void vanka_dump(
    device float* J_array [[buffer(0)]],
    device float* r_array [[buffer(1)]],
    device const float* u [[buffer(2)]],
    device const float* v [[buffer(3)]],
    device const float* H [[buffer(4)]],
    device const float* phi [[buffer(5)]],
    device const float* f_u [[buffer(6)]],
    device const float* f_v [[buffer(7)]],
    device const float* f_H [[buffer(8)]],
