...
class ForwardOperators:
    ...
    def compute_residual(self, dt, 
        ...
        kernel(grid_size, block_size,
               (out_u, out_v, out_ud, out_vd, out_H,
                ...
                state.phi.data, state.psi.data, state.dpsi_dH.data, state.xi.data, state.dxi_dH.data, state.mask.data,
    ...
    def compute_psi(self, relaxation=cp.float32(0.0)):
        ...
        kernel(grid_size, block_size,
                   (grid.state.psi.data,
                    grid.state.dpsi_dH.data,
                    grid.state.dpsi_dbed.data,
                    grid.state.H.data, grid.geometry.depth.data,
                    grid.geometry.sigmoid_c.value,
                    grid.calving.q.data,
                    grid.calving.h0.data,
                    grid.calving.H_c.value,
                    relaxation,
                    grid.ny, grid.nx,
                    stride, halo))
    ...
    def vanka_smooth(self, dt,
        ...
        kernel(grid_size, block_size,
               (self.delta_u, self.delta_v, self.delta_ud, self.delta_vd, self.delta_H,
                state.mask.data,
                state.u.data, state.v.data, state.ud.data, state.vd.data, state.H.data, 
                state.phi.data, state.psi.data, state.dpsi_dH.data, state.xi.data, state.dxi_dH.data,
                self.f_u, self.f_v, self.f_ud, self.f_vd, self.f_H,
    ...
    def vanka_dump(self,dt):
        ...
        kernel(grid_size, block_size,
               (J,r,
                grid.state.u.data, grid.state.v.data, grid.state.ud.data, grid.state.vd.data, grid.state.H.data, grid.state.phi.data, grid.state.psi.data, grid.state.dpsi_dH.data, grid.state.xi.data, grid.state.dxi_dH.data,
                self.f_u, self.f_v, self.f_ud, self.f_vd, self.f_H,
    ...
    def compute_gradient_bed(self):
        ...
        kernel(grid_size, block_size,
               (geometry.bed.grad,
                ...
                state.phi.data, state.psi.data, state.dpsi_dH.data, state.dpsi_dbed.data, state.xi.data, state.dxi_dH.data, state.mask.data,
                geometry.bed.data, 
                rheology.B.data, 
                sliding.beta.data,
                self.gamma,
